...
#SBATCH --mem=70G
#SBATCH --job-name=qwen-resume
#SBATCH --output=qwen-resume_%j.log
...
# Pass an existing complete checkpoint, relative to DIT360/ or absolute.
CHECKPOINT="${1:?Usage: sbatch resume_pano.sh CHECKPOINT}"
shift
[[ -f "$CHECKPOINT/COMPLETE.json" ]] || { echo "Checkpoint is incomplete: $CHECKPOINT" >&2; exit 2; }
...
python -u -m qwen_pano.train \
  ...
  --output "$OUTPUT" --resume "$CHECKPOINT" "$@"
